      ...
      {isOpen && (
        <>
          ...
          <motion.div 
            ...
            className="fixed inset-x-3 top-[7%] z-[101] mx-auto flex max-h-[82vh] w-[min(820px,calc(100vw-24px))] flex-col overflow-hidden rounded-[28px] border border-white/10 bg-[color:var(--surface)] text-white shadow-[0_28px_100px_rgba(0,0,0,0.45)]"
          >
            <div className="flex items-center gap-3 border-b border-white/8 px-4 py-4">
              ...
              <input 
                ...
                className="flex-1 border-none bg-transparent px-1 text-base text-white outline-none placeholder:text-white/30 sm:text-lg"
              />
              <div className="hidden items-center gap-2 rounded-full border border-white/10 bg-white/[0.04] px-3 py-1.5 text-[11px] font-semibold uppercase tracking-[0.18em] text-white/55 md:flex">
                <Command className="h-3.5 w-3.5" />
                Command Center
              </div>
              <button onClick={onClose} className="rounded-2xl border border-white/10 bg-white/[0.04] p-2.5 text-white/50 transition-colors hover:border-white/18 hover:text-white">
                <X size={20} />
              </button>
            </div>

            <div className="flex gap-2 overflow-x-auto border-b border-white/8 px-4 py-3 hide-scrollbar">
              {SCOPES.map((itemScope) => (
                <button 
                  ...
                  className={`whitespace-nowrap rounded-full px-3.5 py-1.5 text-xs font-semibold uppercase tracking-[0.16em] transition-colors ${
                    scope === itemScope
                      ? "bg-white text-background shadow-[0_10px_24px_rgba(255,255,255,0.1)]"
                      : "bg-white/[0.04] text-white/58 hover:text-white"
                  }`}
                >
                  {itemScope}
                </button>
            ...
            <div className="flex items-center justify-between border-b border-white/8 px-4 py-3 text-xs font-semibold uppercase tracking-[0.16em] text-white/45">
            ...
            <div className="flex-1 space-y-2 overflow-y-auto p-3">
              {!deferredQuery ? (
                <div className="grid gap-2 rounded-[24px] border border-white/10 bg-white/[0.03] p-3 md:grid-cols-3">
                  {SEARCH_GUIDES.map((guide) => (
                    <div
                      key={guide}
                      className="rounded-[20px] border border-white/8 bg-white/[0.03] px-3 py-3"
                    >
                      <div className="mb-3 flex h-9 w-9 items-center justify-center rounded-2xl border border-white/8 bg-white/[0.04] text-white">
              ...
              {visibleItems.length ? (
                  ...
                  return (
                    <motion.button
                      ...
                      className={`flex w-full items-center justify-between rounded-[22px] border px-3.5 py-3 text-left transition-all ${
                        isActive
                        ? "border-white/18 bg-white/[0.06] shadow-[0_12px_30px_rgba(0,0,0,0.18)]"
                      ...
                      <div className="flex min-w-0 items-center gap-3">
                        ...
                        <div className="min-w-0">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="rounded-full bg-white/[0.04] px-2 py-1 text-[10px] font-semibold uppercase tracking-[0.16em] text-white/45">
                              {item.group}
                            </span>
                            {isCurrent ? (
                              <span className="rounded-full bg-white px-2 py-1 text-[10px] font-semibold uppercase tracking-[0.16em] text-background">
                                Current
                              </span>
                            ) : null}
                          </div>
                ...
                <div className="flex flex-col items-center justify-center rounded-[24px] border border-dashed border-white/10 bg-white/[0.03] px-6 py-14 text-center">
                  <div className="flex h-12 w-12 items-center justify-center rounded-2xl border border-white/8 bg-white/[0.04] text-white">
                    <Sparkles size={18} />
                  </div>
            ...
            <div className="flex items-center justify-between border-t border-white/10 bg-white/[0.03] px-4 py-3 text-xs font-medium text-white/45">
              <span className="flex items-center gap-2">
                <kbd className="rounded-md border border-white/10 bg-white/[0.04] px-2 py-1">↑</kbd>
                <kbd className="rounded-md border border-white/10 bg-white/[0.04] px-2 py-1">↓</kbd>
                Navigate
              </span>
              <span className="flex items-center gap-2">
